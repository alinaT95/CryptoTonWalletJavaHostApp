...
public class GetOccupiedStorageSizeTest extends KeyChainImmutabilityBaseTest {
    public static final int NUM_OF_ITER = 10;
    public static final int GET_OCCUPIED_STORAGE_SIZE_CORRECT_LC = 64;
    public static final byte GET_OCCUPIED_STORAGE_SIZE_CORRECT_LE = 2;
    ...
    public void testIncorrectSault() throws Exception{
        ...
        for(int i = 0; i < NUM_OF_ITER; i++){
            ...
            try {
                ...
                WalletHostAPI.getWalletCardReaderWrapper().getOccupiedSize(bConcat(sault, computeMac(sault)), GET_OCCUPIED_STORAGE_SIZE_CORRECT_LE);
                assertTrue(false);
            }
            catch (Exception e) {
                e.printStackTrace();
    ...
    public void testWrongLengthLc() throws Exception {
        for(int length = 0; length <= 255; length++){
            ...
            if(length == GET_OCCUPIED_STORAGE_SIZE_CORRECT_LC) continue;
            byte[] data = new byte[length];
            try {
                random.nextBytes(data);
                System.out.println("data = " + ByteArrayHelper.hex(data));
                WalletHostAPI.getWalletCardReaderWrapper().getOccupiedSize(data, GET_OCCUPIED_STORAGE_SIZE_CORRECT_LE);
                assertTrue(false);
            }
            catch (Exception e) {
                e.printStackTrace();
    ...
    public void testWrongLengthLe() throws Exception {
        ...
        for(int le = 0; le <= 255; le++){
            if(le == GET_OCCUPIED_STORAGE_SIZE_CORRECT_LE) continue;
            System.out.println("Le = " + le);
            byte[] sault = walletHostAPI.getSault();
            byte[] mac = computeMac(sault);
            try {
                WalletHostAPI.getWalletCardReaderWrapper().getOccupiedSize(bConcat(sault, mac), (byte) le);
                assertTrue(false);
            }
            catch (Exception e) {
                e.printStackTrace();
    ...
    public void testOneIncorrectMac() throws Exception{
        ...
        try {
            walletHostAPI.getOccupiedStorageSize();
            assertTrue(false);
        }
        catch (Exception e) {
            e.printStackTrace();
