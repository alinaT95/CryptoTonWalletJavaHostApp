...
public class ByteArrayHelper {
    ...
    public static void arrayCopy(byte[] src, int srcOff, byte[] dest, int destOff, int length){
        if (srcOff < 0 || destOff < 0 || length <= 0 ||  src.length < (srcOff + length) || dest.length < (destOff + length))
    ...
    public static int makeShort(byte[] src, int srcOff){
    ...
    public static void setShort(byte[] bArray, short bOff, short sValue){
        if (bOff < 0  ||  bArray.length < (bOff + 2))
            throw new IllegalArgumentException("Bad args!");
    ...
    public static byte[] bytesForShort(short i) {
